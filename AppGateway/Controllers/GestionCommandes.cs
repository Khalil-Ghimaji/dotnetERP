...
namespace AppGateway.Controllers
{
    ...
    public class GestionCommandes : ControllerBase
    {
            ...
            ArticleCommandeRequestDTO articleCommandeRequestDto)
        {
            ...
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var response =
                await _gestionCommandesClient.PostAsync(_gestionCommandesUrl + "ajouterArticle/" + idCommande, content);
            return StatusCode((int)response.StatusCode, await response.Content.ReadAsStringAsync());
        }
            ...
            ArticleCommandeRequestDTO articleCommandeRequestDto)
        {
            ...
            var content = new StringContent(json, Encoding.UTF8, "application/json");
            var response =
                await _gestionCommandesClient.PostAsync(_gestionCommandesUrl + "retirerArticle/" + idCommande, content);
            return StatusCode((int)response.StatusCode, await response.Content.ReadAsStringAsync());
        }

        [HttpPut("Commandes/{idCommande}/ModifierClient")]
        public async Task<ActionResult<CommandeResponseDTO>> ModifierClient(int idCommande,
            CommandeRequestDTO commandeRequestDto)
        {
        ...
        {
            var response =
                await _gestionCommandesClient.PostAsync(_gestionCommandesUrl + "valider/" + idCommande, null);
            var content = await response.Content.ReadAsStringAsync();
            return StatusCode((int)response.StatusCode, content);
        }
        ...
        public async Task<ActionResult<CommandeResponseDTO>> AnnulerCommande(int idCommande)
        {
            ...
            var responseStock =
                await _gestionStockClient.DeleteAsync($"{_gestionStockUrl}annulerReservationCommande/{idCommande}");
